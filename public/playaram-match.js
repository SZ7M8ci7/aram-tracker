...
function championIcon(name) { const info = championInfo(name); return info?.id ? `<img class="playaram-champion-icon match-champion-icon" src="https://ddragon.leagueoflegends.com/cdn/${championVersion}/img/champion/${info.id}.png" alt="${escapeHtml(name)}" />` : `<span class="playaram-champion-fallback match-champion-icon">${escapeHtml(name || "?")}</span>`; }
