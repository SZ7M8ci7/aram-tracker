...
(function expose(root, factory) {
  ...
  function buildStatsFromData(input, query = {}, catalog = { champions: {} }) {
    ...
    const filters = { map: ["mayhem", "aram", "all"].includes(query.map) ? query.map : "mayhem", role: ["all", "Assassin", "Fighter", "Mage", "Marksman", "Support", "Tank"].includes(query.role) ? query.role : "all", from: query.from || null, to: query.to || null };
    ...
    const entries = periodEntries.filter((entry) => mapMatches(entry, filters.map) && roleMatches(entry, filters.role, catalog));
    ...
    for (const entry of entries) {
      const champion = champions.get(entry.champion) || { champion: entry.champion, games: 0, wins: 0, losses: 0, detailedGames: 0, kills: 0, deaths: 0, assists: 0, damageDealt: 0, damageTaken: 0, gold: 0, cs: 0, dpmTotal: 0, dpmGames: 0, gpmTotal: 0, gpmGames: 0, duration: 0, durationGames: 0, lastOrder: entry.order, lastPlayed: entry.playedAt };
      champion.games++; champion.wins += entry.victory ? 1 : 0; champion.losses += entry.victory ? 0 : 1;
      if (entry.order < champion.lastOrder) { champion.lastOrder = entry.order; champion.lastPlayed = entry.playedAt; }
    ...
    const played = new Set(entries.map((entry) => normalizeName(entry.champion))), unusedChampions = Object.values(catalog.champions || {}).filter((champion) => !/^Jade_/i.test(champion.id) && (filters.role === "all" || (champion.roles || []).includes(filters.role)) && !played.has(normalizeName(champion.id))).map((champion) => ({ id: champion.id, name: champion.name, key: champion.id, roles: champion.roles || [] })), wins = entries.filter((entry) => entry.victory).length;
